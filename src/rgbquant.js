...
function RgbQuant(opts) {
  ...
  this.initDist = opts.initDist || 0.1; // play with it 0.01 - 0.1
  // subsequent passes threshold
  this.distIncr = opts.distIncr || 0.05; // play with it 0.005 - 0.05
  // palette grouping
...
RgbQuant.prototype.dither = function (img, kernel, serpentine) {
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (let x = (dir == 1 ? 0 : width - 1), xend = (dir == 1 ? width : 0); x !== xend; x += dir) {
      ...
      // Reduced pixel
      let i32x = this.nearestColor(i32),
        r2 = (i32x & 0xff),
        g2 = (i32x & 0xff00) >> 8,
        b2 = (i32x & 0xff0000) >> 16;
...
RgbQuant.prototype.colorStats2D = function colorStats2D(buf32, width) {
  ...
  boxes.forEach((box) => {
    let effc = Math.max(Math.round((box.w * box.h) / area) * self.boxPxls, 2),
      histL = {},
      col;
...
RgbQuant.prototype.sortPal = function sortPal() {
  ...
  this.idxi32.sort((a, b) => {
    let idxA = self.i32idx[a],
      idxB = self.i32idx[b],
      rgbA = self.idxrgb[idxA],
      rgbB = self.idxrgb[idxB];

    let hslA = rgb2hsl(rgbA[0], rgbA[1], rgbA[2]),
      hslB = rgb2hsl(rgbB[0], rgbB[1], rgbB[2]);

    // sort all grays + whites together
    const hueA = (rgbA[0] == rgbA[1] && rgbA[1] == rgbA[2]) ? -1 : hueGroup(hslA.h, self.hueGroups);
    const hueB = (rgbB[0] == rgbB[1] && rgbB[1] == rgbB[2]) ? -1 : hueGroup(hslB.h, self.hueGroups);

    const hueDiff = hueB - hueA;
    if (hueDiff) return -hueDiff;

    const lumDiff = lumGroup(+hslB.l.toFixed(2)) - lumGroup(+hslA.l.toFixed(2));
    if (lumDiff) return -lumDiff;

    const satDiff = satGroup(+hslB.s.toFixed(2)) - satGroup(+hslA.s.toFixed(2));
...
RgbQuant.prototype.nearestIndex = function nearestIndex(i32) {
  ...
  for (let i = 0; i < len; i++) {
    ...
    const dist = this.colorDist(rgb, this.idxrgb[i]);
...
RgbQuant.prototype.cacheHistogram = function cacheHistogram(idxi32) {
  for (let i = 0, i32 = idxi32[i]; i < idxi32.length && this.histogram[i32] >= this.cacheFreq; i32 = idxi32[i++]) {
...
function stableSort(fn) {
  ...
  if (type == 'Number' || type == 'String') {
    ...
    return this.sort((a, b) => fn(a, b) || ord[a] - ord[b]);
  }
  var ord = this.map(v => v);
...
function isArrSortStable() {
  ...
  return str.split('').sort((a, b) => ~~(str.indexOf(b) / 2.3) - ~~(str.indexOf(a) / 2.3)).join('') == 'xyzvwtursopqmnklhijfgdeabc';
}
...
function makeBoxes(wid, hgt, w0, h0) {
  ...
  for (let y = 0; y < hgt; y += h0) {
    for (let x = 0; x < wid; x += w0) {
      bxs.push({
        x, y, w: (x == xend ? wrem : w0), h: (y == yend ? hrem : h0),
      });
    }
  }
